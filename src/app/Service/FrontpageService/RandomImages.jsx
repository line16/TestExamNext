...
// Function til RANDOM objekt fra API
...
const FrontpageService = () => {
  ...
  useEffect(() => {
    makeRequestImg("http://localhost:5029/images/treatment/");
  }, []);

  useEffect(() => {
    if (dataImg && dataImg.length > 0) {
      const randomImages = getRandomElements(dataImg, 2);
  ...
  return (
    <>
      ...
      {selectedImages.map((e) => (
          <div className="flex flex-row" key={e.id}>
            <figure>
              <Image
